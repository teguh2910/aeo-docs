                        ...
                        <div class="mb-3">
                            ...
                            <?php elseif($userDept === 'management1'): ?>
                                <div class="alert alert-success">
                                    ...
                                    <ul class="mb-0 mt-2">
                                        <li>View questions from <strong>all departments</strong></li>
                                        <li><strong>Process Approval 1</strong></li>
                                        <li>View all documents and validations</li>
                                        <li class="text-muted"><s>Edit/delete questions</s> (Department/AEO/Admin only)</li>
                                        <li class="text-muted"><s>Validate documents</s> (AEO/Admin only)</li>
                            ...
                            <?php elseif($userDept === 'management2'): ?>
                                <div class="alert alert-success">
                                    ...
                                    <ul class="mb-0 mt-2">
                                        <li>View questions from <strong>all departments</strong></li>
                                        <li><strong>Process Approval 2</strong> (after Approval 1)</li>
                                        <li>View all documents and validations</li>
                                        <li class="text-muted"><s>Edit/delete questions</s> (Department/AEO/Admin only)</li>
                                        <li class="text-muted"><s>Validate documents</s> (AEO/Admin only)</li>
...
<?php $__env->startPush('scripts'); ?>
    <script>
        $(document).ready(function() {
            $('#aeoQuestionsTable').DataTable({
                ...
                "columnDefs": [{
                    ...
                    },
                    {
                        "targets": [5, 6, 7, 8,
                            9
                        ], // Detail, Internal Audit, Validasi, Approval, Aksi columns
                        "orderable": false
                    }
                ],
            ...
            // Handle Internal Audit Undo button clicks
            $('.internal-audit-undo-btn').on('click', function() {
                const questionId = $(this).data('question-id');
                const button = $(this);

                if (confirm(
                        'Are you sure you want to undo the Internal Audit Approval for this question? This will reset the approval status to pending.'
                    )) {
                    // Disable button during request
                    button.prop('disabled', true);
                    ...
                    fetch(`/aeo/questions/${questionId}/internal-audit-undo`, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': csrfToken,
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                showToast('Error undoing Internal Audit Approval', 'error');
                                button.prop('disabled', false);
                            }
                        })
                        .catch(error => {
            ...
            // Handle Undo Approval buttons
            $('.undo-approval-btn').on('click', function() {
                const button = $(this);
                const questionId = button.data('question-id');
                const approvalType = button.data('approval-type');

                if (!confirm(
                        `Are you sure you want to undo Approval ${approvalType}? This action will reset the approval status.`
                        )) {
                    return;
                }
                ...
                fetch(`/aeo/questions/${questionId}/undo-approval`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken,
                    ...
                    .then(data => {
                        if (data.success) {
                            showToast(`Approval ${approvalType} undone successfully!`, 'success');
                            ...
                            showToast(data.message || `Error undoing Approval ${approvalType}`,
                            'error');
                            button.prop('disabled', false).html('<i class="fas fa-undo"></i> Undo');
                        }
                    })
                    .catch(error => {
                        console.error('Error:', error);
                        showToast(`Error undoing Approval ${approvalType}`, 'error');
                        button.prop('disabled', false).html('<i class="fas fa-undo"></i> Undo');
                    });
            });
